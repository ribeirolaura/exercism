...
export function monthRate(ratePerHour, discount) {
  let billableMonthDays = 22; 
  let total = 0; 
  let result = (dayRate(ratePerHour) * billableMonthDays); 

  if (discount > 0) {
    let withDiscount = result * discount; 
    total = result - withDiscount; 
...
export function daysInBudget(budget, ratePerHour, discount) {
  let numberDays = budget/monthRate(ratePerHour,discount); 
  let daysInMonth = numberDays * 22; //como retorna o valor unitário de budget por dia, tem que multiplicar por 22. 
  return Math.floor(daysInMonth);
}
